...
function getAdditionalComponentOptions (options) {
  const optionsFlattened = options
    .map(option => {
      ...
      if (option.params) {
        output = output
          .concat(getAdditionalComponentOptions(option.params))
      }
  ...
  const componentNames = optionsFlattened
    .map((option) => option.name)

  // Macro options with duplicates removed
  // For example, Checkboxes have `hint` for fieldset and items
  return optionsFlattened.filter((option, index) =>
    componentNames.indexOf(option.name) === index)
}
    ...
    nestedOptions
      .map(option => {
        ...
        return {
          ...
          name: option.type === 'array' && option.params
